...
const afterSortProducts: Product[] = await page.$$eval('.inventory_item', (items: Element[]) =>
  items.map((item: Element) => ({
    title: (item.querySelector('.inventory_item_label > a') as HTMLAnchorElement)?.textContent || '',
    price: parseFloat((item.querySelector('.inventory_item_price') as HTMLElement)?.textContent?.replace(/\$/g, '') || '0'),
  }))
);

console.log(afterSortProducts);
